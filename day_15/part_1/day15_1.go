...
func getArea(intCode []int) {
	...
	for _, point := range area {
		index := (point.X+lenArea)*1000 + (point.Y + lenArea)
	...
	for _, point := range area {
		...
		if point.up != nil {
			if point.up.content != '#' {
				newIndex := (point.up.X+lenArea)*1000 + (point.up.Y + lenArea)
				graph.AddArc(index, newIndex, 1)
			}
		}
		if point.down != nil {
			if point.down.content != '#' {
				newIndex := (point.down.X+lenArea)*1000 + (point.down.Y + lenArea)
				graph.AddArc(index, newIndex, 1)
			}
		}
		if point.left != nil {
			if point.left.content != '#' {
				newIndex := (point.left.X+lenArea)*1000 + (point.left.Y + lenArea)
				graph.AddArc(index, newIndex, 1)
			}
		}
		if point.right != nil {
			if point.right.content != '#' {
				newIndex := (point.right.X+lenArea)*1000 + (point.right.Y + lenArea)
				graph.AddArc(index, newIndex, 1)
			}
		}
	}
	startIdex := (0+lenArea)*1000 + (0 + lenArea)
	best, err := graph.Shortest(startIdex, oxigenIndex)
	if err != nil {
		log.Fatal(err)
	}
