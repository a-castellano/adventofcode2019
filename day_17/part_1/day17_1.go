...
func getShieldMap(intCode []int) [][]rune {
	input := make(chan int)
...
func calculateAlignmentParameters(shieldMap [][]rune) int {
	var result int
	...
	for i := 0; i < rows; i++ {
		for j := 0; j < columns; j++ {
			if shieldMap[i][j] == '#' {
				if isIntersection(shieldMap, rows, columns, i, j) {
					result += i * j
				}
			}
		}
...
func main() {
	...
	shieldMap := getShieldMap(intCode)
	result := calculateAlignmentParameters(shieldMap)
